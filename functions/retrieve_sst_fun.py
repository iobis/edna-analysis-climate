...
#
################ Retrieve satellite data from Marine Copernicus ################
def retrieve_cop(dataset, variable, username, password, shape, shape_var, shape_filter, time_range,
                ret_type = 'raster', metric = None, group_by_lonlat = False, res_by_month = False,
                grouper = 'time', save_path = None, mask_by_shape = True, k_to_celsius = False, plot = False):
  
  """Retrieve information from Copernicus Marine data products
  
  Retrieve, select and optionally summarise Copernicus satellite data. This function
  also enable to retrieve data from other datasets opened with the function 'inspect_ds'.
  In that case, you should supply the result of the function as the dataset argument,
  following the instructions on the parameters section. If your dataset have 3 dimensions
  (multiple depths), then you should use the retrieve_cop_3d function.
  
  Parameters
  ----------
  dataset : str or list
      Name of the dataset (e.g. METOFFICE-GLO-SST-L4-REP-OBS-SST) or a list containing
  ...
  variable : str
      Name of the variable (e.g. analyzed_sst)
  username : str
      Copernicus username
  password : str
      Copernicus password
  ...
  group_by_lonlat : bool
      ...
      step may be grouped differently by changing the 'grouper' argument.
  res_by_month : bool
  ...
  -------
    ...
    out_path = 'data/saved/'
    retrieve_cop('METOFFICE-GLO-SST-L4-REP-OBS-SST', 'analyzed_sst',
