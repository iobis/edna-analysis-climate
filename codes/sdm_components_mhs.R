...
.cm_load_species <- function(species, species_dataset) {
  ...
  if (tools::file_ext(species_dataset) == "parquet") {
    ...
    con <- DBI::dbConnect(RSQLite::SQLite(), species_dataset)
    
    species_name <- worrms::wm_id2name(species)
    
    sel_species <- paste0("'", species_name, "'", collapse = ", ")
    ...
    db_res <- DBI::dbSendQuery(con, db_query)
    species_data <- DBI::dbFetch(db_res)
    
    DBI::dbDisconnect(con)
    
    species_data_coord <- h3jsr::cell_to_point(species_data$h3)
    species_data_coord <- sf::st_coordinates(species_data_coord)
    colnames(species_data_coord) <- c("decimalLongitude", "decimalLatitude")
    
    species_data <- dplyr::bind_cols(species_data, species_data_coord)
    species_data$data_type <- "fit_points"
    species_data$dataset_sel <- NA
    species_data$taxonID <- species
    
    require(polars)
    fao_areas <- pl$scan_parquet("data/fao_areas.parquet")
    
    sp_sbase <- try(rfishbase::species(list(species_name)))
    ...
    if (!inherits(sp_sbase, "try-error") & nrow(sp_sbase) > 0) {
      ...
      if (nrow(fao_areas_sp) > 0) {
        ...
        if (length(fao_shp_sel) > 0) {
          ...
          is_onarea <- terra::is.related(terra::vect(species_data[,c("decimalLongitude", "decimalLatitude")], geom = c("decimalLongitude", "decimalLatitude")),
                                         terra::vect(fao_shp_sel),
                                         "intersects")
          is_onarea <- ifelse(is_onarea, 1, 0)
          
          species_data$fao_confirmed <- is_onarea
        } else {
          species_data$fao_confirmed  <- NA
        }
    ...
    if (!"fao_confirmed" %in% colnames(species_data)) {
      species_data$fao_confirmed <- NA
    }
