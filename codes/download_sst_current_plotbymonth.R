...
ggplot(sites_metrics, aes(x = date, y = mean))+
  geom_line() +
...
ggplot(sites_metrics, aes(x = date, y = detrend))+
  geom_hline(yintercept = 0) +
  #geom_line(aes(color = state)) +
  geom_area(aes(y=ifelse(detrend<0, detrend, 0)), fill="#1093C8") +
  geom_area(aes(y=ifelse(detrend>0, detrend, 0)), fill="#C72B10") +
  theme_bw() +
  facet_wrap(~ MRGID, scales = "free_y")
