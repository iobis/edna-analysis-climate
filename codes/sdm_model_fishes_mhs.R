...
library(dplyr)
library(terra)
source("codes/sdm_components_mhs.R")
source("codes/sdm_model_function_mhs.R")
...
# General
# The output folder
outfolder <- check_exists("outfolder", "~/Research/obis/research/marineheritage_sst/results/sdms")
# An unique code for identifying this model run
outacro <- check_exists("outacro", "mhs")
...
species_dataset <- "~/Research/mpa_europe/mpaeu_shared/occurrence_h3_db.sqlite"
# The path for the species list
species_list <- "~/Research/obis/research/marineheritage_sst/results/fish_paf_species.csv"#"~/Research/obis/research/marineheritage_sst/results/fish_paf_sdmlist.csv"
...
if (run_parallel) {
  ...
  with_progress({
    ...
    result <- future_map2(species_list$AphiaID, species_list$sdm_group, pmod,
  ...
  with_progress({
    p <- progressor(steps = nrow(species_list))
    result <- purrr::map2(species_list$AphiaID, species_list$sdm_group, pmod,
                          sdat = species_dataset, outf = outfolder,
                          outac = outacro, alg = algos, lmd = limit_by_depth,
                          lmd_buf = depth_buffer, assb = assess_bias,
                          corb = correct_bias,
